...
class InsuranceOrderAdmin extends Admin {
	...
	public function configureFormFields(FormMapper $form)
	{
		$form->with('General')
			...
			->add('birthDate', null, array('label' => 'Дата рождения', 'years' => range(date('Y')-70, date('Y') )))
			->end()
			->with('Pay')
			...
			->add('documentInn', null, array('label' => 'ИНН'))
	...
	public function configureDatagridFilters(DatagridMapper $filter)
	{
		$filter->add('id')
			...
			->add('user', null, array('label' => 'Пользователь'))
			->add('company', null, array('label' => 'Страховая'))
			->add('city', null, array('label' => 'Город проживания'))
    ...
    public function preRemove($object)
    {
		$policy = $this->getConfigurationPool()->getContainer()->get('doctrine')
		->getRepository('InsuranceContentBundle:Policy')
		->findOneById($object->getPolicy()->getId());
